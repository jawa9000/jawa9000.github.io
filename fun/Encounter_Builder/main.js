...
$(document).ready(function() { // listeners
    ...
    // Add the environment dropdown population here
    if (typeof monsters !== 'undefined') {
        const envSet = new Set();
        monsters.forEach(mon => {
            if (Array.isArray(mon.environments)) {
                mon.environments.forEach(env => {
                    if (env && typeof env === 'string') envSet.add(env);
                });
            }
        });
        const envArr = Array.from(envSet).sort();
        const $envSelect = $('#environment');
        $envSelect.empty();
        $envSelect.append('<option>Any</option>');
        envArr.forEach(env => {
            $envSelect.append(`<option>${env}</option>`);
        });
    }
