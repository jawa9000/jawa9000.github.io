...
$('#start').on('click', function() { // start timer and display math problems
    ...
    if (started == false) { // if starting the math problems for the first time, generate the math problems. Otherwise, display the existing math problems.
        displayProblems('ordering'); // add new problems
        started = true;
    } else {
...
$('#reset').on('click', function() { // reset timer and generate new math problems
    ...
    displayProblems(operation); // generate new problems
});
...
function generateProblems(num, largest, operation, negativeNumbers) { // generate a "table" of subtraction problems
    ...
    for (var i = 0; i < num * 10; i++) {
        numberArray.push(generateNumber(largest, false)); // add new random number

        if (checkIfDuplicateExists(numberArray)) { // check for duplicate number and remove it
            while (checkIfDuplicateExists(numberArray)) {
                numberArray = uniq(numberArray);
            }
        }
    }

    var finalArray = []; // proper order
    var randomArray = []; // random order 

    for (var i = 0; i < num; i++) {
        var picked = generateNumber(numberArray.length - 1); // pick a random position from numberArray
        finalArray.push(numberArray[picked]); // push randomly select number into finalArray
        randomArray.push(numberArray[picked]);
        numberArray.splice(picked, 1); // remove randomly select number from numberArray
    }

    finalArray = finalArray.sort(function (a,b) { // sort array with lowest to highest numbers
        return a - b;
    });

    for (i in finalArray) {
        output += '<div class="block answer">' + i + '</div>';
    }
    ...
    for (k in finalArray) { // generate html
        for (j in randomArray) {
            if (finalArray[k] == randomArray[j]) {
                output += '<div class="block draggable" order="' + j + '">' + finalArray[j] + '</div>';
                break;
            }
        }
    }
...
// ** Add drag and drop routine
// ** get the jquery drag and drop plugin